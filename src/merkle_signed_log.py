...
class MerkleSignedLog:
    @staticmethod
    def _commitment_message(root_hash: str, key_id: str, tree_size: int) -> bytes:
        payload = {
            "scheme": "auditmesh_merkle_signed_v2",
            "root_hash": root_hash,
            "key_id": key_id,
    ...
    def commit(events: List[Event], private_key_path: str, key_id: str) -> MerkleCommitment:
        ...
        signature = private.sign(MerkleSignedLog._commitment_message(root, key_id, len(leaves))).hex()
        return MerkleCommitment(
            root_hash=root,
    ...
    def verify(events: List[Event], commitment: MerkleCommitment, registry: KeyRegistry) -> Tuple[bool, int]:
        ...
        try:
            pub.verify(
                ...
                MerkleSignedLog._commitment_message(commitment.root_hash, commitment.key_id, commitment.tree_size),
            )
        except Exception:
            return False, 0
    ...
    @staticmethod
    def verify_commitment_signature(commitment: MerkleCommitment, registry: KeyRegistry) -> bool:
        if commitment.key_id in registry.revoked_keys:
            return False
        pub_path = registry.keys.get(commitment.key_id)
        if not pub_path or not os.path.exists(pub_path):
